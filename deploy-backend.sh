...
ssh ec2-user@commerce-api.et3am.com "cd /home/ec2-user/et3am && git fetch origin master && git reset --hard origin/master && cd backend && npm install && npm run build"

echo "🔄 Starting AWS backend with ecosystem.config.js..."
ssh ec2-user@commerce-api.et3am.com "cd /home/ec2-user/et3am && pm2 stop et3am-backend 2>/dev/null || true && pm2 delete et3am-backend 2>/dev/null || true && pm2 start ecosystem.config.js"
